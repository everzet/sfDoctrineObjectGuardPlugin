...
$community->addUserToGroup($guardUser, $participantGroup);
$queriesCount = $conn->count();
$t->ok(
  $user->hasCredential('community/' . $community->getId() . '/add_publication'),
  'User now "have right to add publication" into community'
);
$t->is($queriesCount, $conn->count() - 1, '"One query" fired to knew');
$t->ok($user->hasCredentialForObject('add_publication', $community), 'hasCredentialForObject() returns true on "add_publication"');
$t->is($queriesCount, $conn->count() - 1, '"No query" fired to knew');
$t->ok(!$user->hasCredentialForObject('edit_publication', $community), 'hasCredentialForObject() returns false on "edit_publication"');
$t->ok(
  !$user->hasCredential('community/' . $community->getId() . '/edit_publication'),
  'User "have no right to edit publication" in community'
);
$t->is($queriesCount, $conn->count() - 1, '"No query" fired to knew');
$t->ok($user->isCredentialsLoadedForTable(Doctrine::getTable('Community')), '"Credentials loaded" for community');
$community->setOwner($guardUser);
$community->save();
$t->ok(
  !$user->hasCredential('community/' . $community->getId() . '/edit_publication'),
  'User "still have no right to edit publication" in community'
);
$user->reloadCredentialsForTable(Doctrine::getTable('Community'));
$queriesCount = $conn->count();
$t->ok(
  $user->hasCredential('community/' . $community->getId() . '/edit_publication'),
...
$t->ok($user->hasCredentialForObject('add_publication', $community), 'hasCredentialForObject() also returns true');
